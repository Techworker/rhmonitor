...
function print() {
    ...
    let output = "";
    output += '╔═════════════════════╤══════╤═════════╤══════════════╤═════════╤══════╤══════╗' + "\n";
    output += '║ Server              │ Name │ Threads │ Speed        │ ACC/REJ │ Temp │ Fan  ║' + "\n";
    output += '╟─────────────────────┼──────┼─────────┼──────────────┼─────────┼──────┼──────╢' + "\n";
    ...
    Object.keys(servers).forEach((k) => {
        ...
        if(server.data === null || server.errorMsg !== null) {
            all.bad++;
            let ip = clc.bold(format(server.ip, false, 19));
            output += `║ ${ip} │ xxxx │ xxxxxxx │ xxxxxxxxxxxx │ xxx/xxx │ xxxx │ xxxx ║` + "\n";
            output += '╟─────────────────────────────────────────────────────────────────────────────╢' + "\n";
            let msg;
            if(server.inited === true) {
                msg = clc.bgRed.white(format(server.errorMsg, false, 64));
            } else {
                msg = clc.bgYellow.white(format(server.errorMsg, false, 64));
            }
        ...
        server.data.infos.forEach((info, idx) => {
            const ip = clc.bold(format(first ? server.ip : '', false, 19));
            first = false;
            ...
            const speed = format(info.speed + ' H/s', true, 12);
        ...
        if(!server.compact) {
            ...
            let msg = format(`Uptime: ${uptime}  ${server.data['stratum.user']}@${server.data['stratum.server']} ${server.data.extrapayload}`, false, 64);
            let msg2 = format("", false, 75);
            output += '║ ' + msg2 + ' ║' + "\n";
